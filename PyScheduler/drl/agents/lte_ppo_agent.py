"""
PPO-агент для LTE-планирования на structured observation из PyScheduler.

Модуль повторяет ключевую идею `lte_dqn_agent.py`: per-UE признаки кодируются
общим энкодером, затем объединяются с pooled-представлением активных UE и
глобальным контекстом. В отличие от DQN, здесь поверх этих признаков строится
actor-critic policy c PPO-обновлением и обязательной поддержкой action-mask.
"""
...
@dataclass
class MaskedPPORolloutBatch:
    """
    Полный rollout-батч для PPO-обновления с сохранением action-mask.
    """
...
class MaskedPPORolloutBuffer:
    """
    Последовательный буфер rollout для PPO.

    PPO учится не на replay buffer, а на коротких свежих траекториях. Поэтому
    здесь буфер просто накапливает шаги очередного rollout и очищается после
    обновления policy.
    """
    ...
    def as_batch(self) -> MaskedPPORolloutBatch:
        return MaskedPPORolloutBatch(
...
class SharedUEActorCritic(nn.Module):
    """
    LTE-специфичная actor-critic сеть с общим UE-энкодером.

    Actor выдает логиты по всем слотам UE, critic оценивает value всего
    состояния. Для pooled-представления учитываются только активные действия
    из action-mask, поэтому critic и actor видят одинаковый набор активных UE.
    """
    ...
    def _reset_parameters(self) -> None:
        ...
        nn.init.orthogonal_(final_policy.weight, gain=0.01)
        nn.init.zeros_(final_policy.bias)
...
class LTEPPOAgent:
    """
    LTE PPO-агент для обучения и инференса на fixed-width observation.
    """
    ...
    def __init__(
        ...
        self.policy = SharedUEActorCritic(
            max_n_ue=max_n_ue,
    ...
    def _mask_policy_logits(
        ...
        sanitized_mask = LTEPPOAgent._sanitize_action_mask_tensor(action_mask)
        return policy_logits.masked_fill(~sanitized_mask, -1e9)
    ...
    def _distribution(
        ...
        policy_logits, value = self.policy(obs, sanitized_mask)
        masked_logits = self._mask_policy_logits(policy_logits, sanitized_mask)
    ...
    def select_action(
        ...
        with torch.no_grad():
            dist, value = self._distribution(obs_tensor, mask_tensor)
            if deterministic:
    ...
    def estimate_value(
        ...
        with torch.no_grad():
            _, value = self._distribution(obs_tensor, mask_tensor)
    ...
    def update(
        ...
        for _ in range(self.n_epochs):
            ...
            for start_idx in range(0, total_size, self.batch_size):
                ...
                dist, values_new = self._distribution(mb_states, mb_masks)
