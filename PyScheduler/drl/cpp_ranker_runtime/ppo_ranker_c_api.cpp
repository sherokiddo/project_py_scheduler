...
int ppo_ranker_create(const char* model_path, void** out_handle) {
    try {
        ...
        auto* runtime = new PpoRankerRuntime(std::string{model_path});
        *out_handle = runtime;
        g_last_error.clear();
        return 0;
...
int ppo_ranker_predict(
    ...
    try {
        ...
        auto* runtime = static_cast<PpoRankerRuntime*>(handle);
...
void ppo_ranker_destroy(void* handle) {
    auto* runtime = static_cast<PpoRankerRuntime*>(handle);
    delete runtime;
}

const char* ppo_ranker_last_error() {
