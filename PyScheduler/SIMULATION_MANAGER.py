...
class SimulationManager:
    ...
    def enable_stats_log(self):
        """
        Включить legacy-логирование статистики симуляции в `stats.csv`.
        """
    ...
    def start_simulation(self) -> None:
        ...
        Основные этапы:
            ...
            4. Инициализация менеджера статистики.
            5. Основной цикл по времени (TTI).
            6. Завершение симуляции и экспорт статистики.
        """
        ...
        try:
            ...
            # Повторный запуск на том же инстансе не должен смешивать логи между прогонами.
            self.harq_log = {
                "tti": [],
                "ue_id": [],
                "tx_count": [],
            ...
            if hasattr(self, "_stats_file_initialized"):
                delattr(self, "_stats_file_initialized")

            if self.sched_config.harq_enabled:
                warnings.warn(
                    "SchedulerConfig.harq_enabled currently keeps backward-compatible config only: "
                    "SimulationManager does not yet populate HARQ feedback/retransmission logs in "
                    "the active run_tti pipeline.",
                    UserWarning,
                )
            ...
            for tti in range(self.sim_config.sim_duration):
                ...
                if (tti % stats_refresh_tti == 0) and self.scheduler:
                    ...
                    tput = amc_stats.get("dl_throughput_sum_kbps", 0)
                    prb = sched_stats.get("dl_prb_utilization_pct", 0.0)
                    ue_cnt = sched_stats.get("sch_active_ue_count", 0)
                    total_time_us = sched_stats.get("sch_total_time_us", 0.0)
                    jfi_str = f"{jfi_cached:.4f}" if jfi_cached is not None else "N/A"

                    sbar.set_description_str(
                        f"TTI={tti} | UE={ue_cnt} | "
                        f"Tput={tput:.0f} kbps | PRB={prb:.1f}% | "
                        f"JFI(long)={jfi_str} | sch_total_time_us={total_time_us:.1f}"
                    )
                    sbar.refresh()
            ...
            if sbar is not None:
                sbar.close()
            if self._to_file and self._log_file is not None:
    ...
    def _stats_logging(self, sched_result: dict) -> None:
        ...
        with open(filename, "a", newline="", encoding="utf-8") as file:
            ...
            for ue in self.ue_collection.GET_ALL_USERS():
                ...
                rbs = allocation.get(ue_id, [])
                num_cce = pdcch_allocation.get(ue_id, 0)
                tx_bits = int(ue.current_dl_throughput / 1000)

                buf_size = 0
                if (
                    hasattr(self.base_station, "buffer_manager")
                    and hasattr(self.base_station.buffer_manager, "buffers")
                    and ue_id in self.base_station.buffer_manager.buffers
                ):
                    buf_size = self.base_station.buffer_manager.buffers[ue_id].current_size * 8
                elif (
                    hasattr(self.base_station, "ue_buffers")
                    and ue_id in self.base_station.ue_buffers
                    and hasattr(self.base_station.ue_buffers[ue_id], "sizes")
                ):
                    buf_size = self.base_station.ue_buffers[ue_id].sizes.get(ue_id, 0) * 8
    ...
    def plot_harq_results(self, x_max=None, y_max=None, window=100):
        ...
        if not tti_list:
            print(
                "HARQ log is empty. The current SimulationManager pipeline does not "
                "populate HARQ transmission history."
            )
            return
        ...
        axes[1].plot(
            ...
            success_rate_smooth,
            linewidth=2,
            label=f"Moving average ({window} TTI)",
        )
