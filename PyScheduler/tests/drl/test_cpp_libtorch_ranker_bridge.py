...
from drl.cpp_libtorch_ranker_bridge import (
    CppPPORankerBridge,
    normalize_cpp_runtime_search_paths,
)
...
def _workspace_tmp_dir() -> Path:
    ...
    run_dir = base_dir / f"cpp_libtorch_ranker_bridge_{uuid4().hex}"
    run_dir.mkdir()
    return run_dir
...
class _FakeLib:
    def __init__(self):
        ...
        self.ppo_ranker_create = _FakeFn(self._create)
        self.ppo_ranker_predict = _FakeFn(self._predict)
        self.ppo_ranker_destroy = _FakeFn(self._destroy)
        self.ppo_ranker_last_error = _FakeFn(self._last_error)

    def _create(self, model_path, out_handle):
        assert model_path.endswith(b".ts")
        handle_ptr = ctypes.cast(out_handle, ctypes.POINTER(ctypes.c_void_p))
        handle_ptr[0] = ctypes.c_void_p(1234)
        return 0
...
def test_cpp_libtorch_ranker_bridge_predict_scores(monkeypatch):
...
def test_normalize_cpp_runtime_search_paths():
    paths = normalize_cpp_runtime_search_paths(
        ["D:/libtorch/lib", Path("D:/runtime"), "D:/libtorch/lib"]
    )
    assert paths == [Path("D:/libtorch/lib"), Path("D:/runtime")]
