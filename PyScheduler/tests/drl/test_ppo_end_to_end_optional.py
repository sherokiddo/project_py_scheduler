...
from drl.scripts.train_lte_ppo_scheduler import make_env
...
def test_runtime_ppo_train_to_inference_end_to_end(monkeypatch):
    weights_path = PYSCHEDULER_PPO_RUN_DIR / f"smoke_lte_ppo_{uuid4().hex}.pt"
    ...
    try:
        ...
        assert manager.scheduler.__class__.__name__ == "PpoScheduler"
        stats = manager.scheduler.get_stats()
        assert stats["ppo_step_count"] > 0
        assert Path(stats["ppo_model_path"]) == weights_path
