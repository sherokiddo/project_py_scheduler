...
from HARQ_MANAGER import HARQIface, HARQ_State
...
class SchedulerInterface:
    ...
    def check_HarqProcess(self, ue_id, harq_iface):

        if not harq_iface.enabled:
            return {"state": "IDLE", "process_id": None}

        processes = harq_iface._ue_processes.get(ue_id, [])

        for proc in processes:
            if proc.state == HARQ_State.FAILED:
                return {
                    "state": "FAILED",
                    "process_id": proc.process_id
                }
            
            if proc.state == HARQ_State.WAIT_RETX:
                return {
                    "state": "WAIT_RETX",
                    "process_id": proc.process_id
                }
    ...
    def _filter_eligible_ues(self, tti: int, users: List[Dict]) -> List[Dict]:
        ...
        for user in users:
            ...
            if buffer_size > 0 and 1 <= user_cqi <= 15:
                ...
                harq_status = self.check_HarqProcess(user['UE_ID'], self.harq_manager)
...
class AdaptiveModulationAndCoding:
    ...
    def GET_MCS_FROM_CQI(self, cqi: int) -> int:
        ...
        if cqi < 0 or cqi >= len(GLOBALS.CQI_TO_SE):
            return 0

        target_se = GLOBALS.CQI_TO_SE[cqi]

        mcs = 0

        for i, se in enumerate(GLOBALS.MCS_TO_SE):
    ...
    def GET_ITBS_FROM_MCS(self, mcs: int) -> int:
        """
        MCS -> ITBS
        """
        if mcs < 0 or mcs >= len(GLOBALS.MCS_TO_ITBS):
            return 0

        return GLOBALS.MCS_TO_ITBS[mcs]

    def GET_TBS_FROM_ITBS(self, itbs: int, n_prb: int) -> int:

        if itbs < 0 or itbs >= len(GLOBALS.LTE_TBS_TABLE):
            return 0

        if n_prb < 1:
            return 0

        row = GLOBALS.LTE_TBS_TABLE[itbs]
    ...
    def GET_TBS_FROM_CQI(self, cqi: int, n_prb: int) -> int:

        mcs = self.GET_MCS_FROM_CQI(cqi)

        itbs = self.GET_ITBS_FROM_MCS(mcs)

        return self.GET_TBS_FROM_ITBS(itbs, n_prb)
