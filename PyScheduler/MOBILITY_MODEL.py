...
class DiagonalWalkModel:
    def __init__(self, bs_x: float, bs_y: float):
        """
        Инициализация модели движения через соту.

        Args:
            bs_x: Координата X базовой станции
            bs_y: Координата Y базовой станции
        """
        self.bs_x = bs_x
        self.bs_y = bs_y
        self._first = True
    ...
    def update(
        ...
        """
        Обновляет позицию устройства на основе движения через соту.
        ...
        Args:
            ...
            mean_direction: Среднее направление
            time_ms: Время в миллисекундах

        Returns:
            new_position: Новые координаты устройства (x, y)
            new_velocity: Новая скорость (м/с)
            new_direction: Новое направление
            mean_velocity: Средняя скорость (м/с)
            mean_direction: Среднее направление
        """
        dx = self.bs_x - current_position[0]
        dy = self.bs_y - current_position[1]
        dist = np.hypot(dx, dy)

        if dist == 0:
            new_pos = (self.bs_x, self.bs_y)
        else:
            ux = dx / dist
            uy = dy / dist
            move = 2 * dist
            new_pos = (current_position[0] + ux * move, current_position[1] + uy * move)

        new_velocity = current_velocity
        new_direction = np.arctan2(dy, dx)

        return new_pos, new_velocity, new_direction, mean_velocity, mean_direction
